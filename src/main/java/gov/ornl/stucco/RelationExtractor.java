...
public class RelationExtractor {
	 ...
	 */
	public String createSubgraph(String source, Annotation doc) {
		...
		for (int i=0; i<sentences.size(); i++) {
			...
			List<CyberEntityMention> entities = sentence.get(CyberEntityMentionsAnnotation.class);
			if ((entities != null) && (!entities.isEmpty())) {
				List<Vertex> newVertices = createVertices(entities, source);
				vertices.put(Integer.valueOf(i), newVertices);
			}
		}
		
		edges = createEdges(vertices, source);
		
		StringBuilder graphBuilder = new StringBuilder();
		graphBuilder.append("{ \"vertices\": [");
		for (Integer key : vertices.keySet()) {
			List<Vertex> vList = vertices.get(key);
			for (int i=0; i<vList.size(); i++) {
				Vertex v = vList.get(i);
				graphBuilder.append(v.toGraphSON());
				if (i < vList.size() - 1) {
					graphBuilder.append(", ");
				}
			}
	...
	public List<Vertex> createVertices(List<CyberEntityMention> entities, String source) {
		List<Vertex> vertices = new ArrayList<Vertex>();
		
		for (CyberEntityMention cyberEntity : entities) {
			boolean vertexUpdated = false;
			if (vertices.size() > 0) {
				for (Vertex v : vertices) {
					//Assume that same type mentions in the same sentence are values to the same vertex
					if ((v.getVertexType().equalsIgnoreCase(cyberEntity.getType())) && (v.getProperty(cyberEntity.getSubType()) == null)) {
						v.addProperty(cyberEntity.getSubType(), cyberEntity.getValue());
						vertexUpdated = true;
						break;
					}
				}
			...
			if (!vertexUpdated) {
				Vertex vertex = new Vertex(cyberEntity.getObjectId(), cyberEntity.getType());
				vertex.addProperty(cyberEntity.getSubType(), cyberEntity.getValue());
				vertex.addProperty(SOURCE_PROPERTY, source);
				vertices.add(vertex);
			}
		}
	...
	public List<Edge> createEdges(Map<Integer, List<Vertex>> vertices, String source) {
		List<Edge> edges = new ArrayList<Edge>();
		Vertex v1 = null;
		
		for (Integer key : vertices.keySet()) {
			ListIterator<Vertex> vList = vertices.get(key).listIterator();
			//Look for related entities within the same sentence
			if (vList.hasNext()) {
				v1 = vList.next();
			}
			while (vList.hasNext()) {
				Vertex v2 = vList.next();
				if ((v1.getVertexType().equalsIgnoreCase("software")) && (v2.getVertexType().equalsIgnoreCase("vulnerability"))) {
					String id = v1.get_id() + "_" + v2.get_id();
					Edge e = new Edge(id, "hasVulnerability", v2.get_id(), v2.getVertexType(), v1.get_id(), v1.getVertexType(), source);
					edges.add(e);
				}
				else if ((v2.getVertexType().equalsIgnoreCase("software")) && (v1.getVertexType().equalsIgnoreCase("vulnerability"))) {
					String id = v2.get_id() + "_" + v1.get_id();
					Edge e = new Edge(id, "hasVulnerability", v1.get_id(), v1.getVertexType(), v2.get_id(), v2.getVertexType(), source);
					edges.add(e);
				}
